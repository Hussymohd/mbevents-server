...
const registerUser = async (req, res) => {
  ...
  try {
    //check if the mail/user has been registered or not
...
const forgotPassword = async (req, res) => {
  ...
  //save the reset token and its epiry in the db
  try {
...
const resetPassword = async (req, res) => {
  ...
  if(!token || !newPassword) {
    return res.status(400).json ({success: false, message: "Provide token and new passowrd"})
  }
  ...
  try {
    ...
    await user.save()
     res.status(200).json({ success: false, message: "Password has been reset successfully" });
  } catch (error) {
